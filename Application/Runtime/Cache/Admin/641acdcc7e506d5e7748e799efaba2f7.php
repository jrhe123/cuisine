...
<div class="page-container">
            ...
            <div class="portlet box blue-hoki">
                ...
                <div class="portlet-body">
                    <?php if(($_SESSION['cooking_admin_role']) == "admin"): ?><ul class="nav nav-tabs">
                            <?php if(is_array(C("CASE_AUDIT_STATUS"))): foreach(C("CASE_AUDIT_STATUS") as $key=>$audit_status): if($_GET['audit_status']== $key): ?><li class="active">
                                        <a href="?audit_status=<?php echo ($key); ?>"><?php echo ($audit_status); ?></a>
                                    </li>
                                    <?php else: ?>
                                    <li>
                                        <a href="?audit_status=<?php echo ($key); ?>"><?php echo ($audit_status); ?></a>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                <h4 class="modal-title">您确定要删除该帖子？操作将无法恢复！</h4>
            </div>
            <div class="modal-body">
                <p>您确定要删除该帖子？操作将无法恢复！</p>
            </div>
            <div class="modal-footer">
                <button type="button" data-dismiss="modal" class="btn">关闭</button>
                <!--<button type="button" data-dismiss="modal" class="btn red" onclick="noPassAudit()">不通过审核</button>-->
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                <h4 class="modal-title">您确定要显示该帖子？</h4>
            </div>
            <div class="modal-body">
                <p>您确定要显示该帖子？</p>
            </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                <h4 class="modal-title">您确定要隐藏该帖子？</h4>
            </div>
            <div class="modal-body">
                <p>您确定要隐藏该帖子？</p>
            </div>
...
<script>
    ...
    <script>
        ...
        jQuery(document).ready(function () {
            ...
            <?php if(is_array($property)): foreach($property as $key=>$_property): ?>$('input:checkbox[value="<?php echo ($_property); ?>"]').attr('checked', true);<?php endforeach; endif; ?>
            $.uniform.update();
            $("input[name='participant']").select2({
                tags: [<?php echo ($admins); ?>]
            });
...
//            $('select[name="company_status"]').val('<?php echo ($company_status); ?>');
            ...
            grid = ajaxDataTableJs('dataTable', '<?php echo U("tableData");?>?audit_status=<?php echo ($_GET['audit_status']); ?>', order, columns);
        ...
        // 监听状态下拉菜单的改变
        function statusSelectChange(obj) {
            var status = $(obj).val();
            if (status == 4) {
                $('#closeWayGroup').show();
            } else {
