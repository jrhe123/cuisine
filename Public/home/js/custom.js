jQuery(document).ready(function ($) {
    ...
    jQuery(".form-js").submit(function () {
        ...
        if(user_id != ''){
            name = jQuery('input[name="user_name"]').val();
        }
        var data = {'name': name, 'email': mail,'content': message,'user_id':user_id};
        if (name == "") {
            jQuery("#name").after('<span class="form-description required-error">请输入内容</span>');
        } else {
            jQuery("#name").parent().find('.required-error').remove();
        }
        if (mail == "") {
            jQuery("#mail").after('<span class="form-description required-error">请输入内容</span>');
        } else {
            jQuery("#mail").parent().find('.required-error').remove();
        }
        if (message == "") {
            jQuery("#message").after('<span class="form-description required-error">请输入内容</span>');
        } else {
            jQuery("#message").parent().find('.required-error').remove();
        }

        if (name != "" && mail != "" && message != "") {
            jQuery.post("/Home/Contact/feedback", data, function (data) {
                if (data.code == 200) {
                    thisform.prepend("<div class='alert-message success'><i class='fa fa-thumbs-o-up'></i><p><span>感谢 " + name + "留言!</span><br>我们会尽快联系</p></div>");
                    jQuery("#name").val("");
                    jQuery("#mail").val("");
                    jQuery("#message").val("");
                }
    ...
    window.onload = function () {
        ...
        var header = parseFloat(jQuery("#header-top").outerHeight() + jQuery("#header").outerHeight() + jQuery("#baidumap").outerHeight() + 80);
